...
def visible(turtle_name: turtle.Turtle()):  # returns true if both the x- and y-value of the turtle's position are between -480 and 480
    # you will need this: x-value: turtle_name.position()[0]
    # and this:           y-value: turtle_name.position()[1]

    x: bool = turtle_name.position()[0] >= -480 and turtle_name.position()[0] <= 480
    y: bool = turtle_name.position()[1] >= -480 and turtle_name.position()[1] <= 480
...
def star_pattern(num):
    ...
    skip = num // 2 if num % 2 != 0 else num - 3
    angle = 360 / num * skip
